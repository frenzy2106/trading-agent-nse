...
2a. **Check for a pre-extracted fact card below.** If a section titled `## Pre-extracted fact card (authoritative)` is present in this system prompt, it was generated by structured extraction from the most recent earnings call. Every field carries a verbatim source quote. Treat its headline financials, segments, order book, and guidance as **ground truth for that period** — do NOT re-derive them via `get_management_commentary`. Cross-reference them against `get_fundamentals_snapshot` (which gives reported numbers from yfinance) and surface contradictions explicitly. For themes / risks / segment narratives / capex timelines / geographic split, call `get_fact_card_detail(ticker, section)` instead of fishing through RAG.

   **Segment-scoping discipline (critical):** Each segment's data in the fact card is strictly scoped to that segment. Facility names, utilisation figures, order numbers, and developments listed under one segment apply ONLY to that segment. Never attribute a fact from one segment to a different segment — for example, if Industrial Gas mentions a Kalol/Kandla facility at 85% utilisation, do NOT assume the Beverage Keg plant is at 85%. If a question is about a specific segment and the fact card lacks that fact, say so explicitly or call `get_fact_card_detail(ticker, "segments_detail")` for the full per-segment narrative — do not infer across segments.
